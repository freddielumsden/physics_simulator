...
impl Ball {
    fn update(&mut self) {
        ...
        if self.x_pos+self.radius > screen_width() {
            self.x_vel = -self.x_vel*self.boundary_cor;
            self.x_pos = screen_width()-self.radius;
        ...
        if self.y_pos+self.radius > screen_height(){
...
fn collide(ball_1: &mut Ball, ball_2: &mut Ball) {
    ...
    if distance < ball_1.radius + ball_2.radius {
        ...
        /*
        let overlap = (ball_1.radius + ball_2.radius)/2.0;
...
async fn main() {
    ...
    for i in 0..2 {
        balls.push(Ball {
            ...
            radius: 15.0,
            x_pos: (i as f32)/1000.0,
            y_pos: (i as f32)/1000.0,
            x_init_pos: (i as f32),
            y_init_pos: (i as f32),
            x_vel: 30.0,
            y_vel: (i as f32) * 10.0,
            x_init_vel: (i as f32) * 10.0,
            y_init_vel: (i as f32) * 10.0,
            x_acc: 0.0,
            y_acc: G,
            time: 0.0,
            boundary_cor: 0.9,
